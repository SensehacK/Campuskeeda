...
<title>Manage B.E Projects || Kampus keeda</title>
...
<td><?php include("top.php"); ?></td>
 ...
 <?php 
  ...
  {
		$sql="delete from campues_beprojects where id='$_REQUEST[id]'";
		if (!mysql_query($sql,$con))
		{
			die('Error: ' . mysql_error());
		}
		echo"<meta http-equiv=refresh content=\"0;url=manage_beprojects.php?action=view\">";
  }
  if (($_REQUEST['action']=='active') && ($_REQUEST['id']!=''))
  {
		$status=$_REQUEST['status'];	
		$id=$_REQUEST['id'];
		$sql="update campues_beprojects set status='$status' where id='$id'";
		if (!mysql_query($sql,$con))
		{
			die('Error: ' . mysql_error());
		}
  ...
  if ($_REQUEST['action']=='save')
  {
		...
		if($_FILES['projects']['name'] != '')
		{
				...
				{
					echo "<script langauge=\"javascript\">alert(\"Sorry you can not upload this Image.\");location.href='manage_beprojects.php?action=view';</script>";
					return;
				}
		}	
		$sql="INSERT INTO campues_beprojects (title,projects,post_date) VALUES ('$title','$projects','$post_date')";
		if (!mysql_query($sql,$con))
			{
				die('Error: ' . mysql_error());
			}
  ...
  if (($_REQUEST['action']=='update')&&($_REQUEST['id']!=''))
  {
...

		{
				...
				{
					$projects = $temp_code.'_'.$_FILES['projects']['name'];
					$sql="update campues_beprojects set projects='$projects' where id='$id'";
		...
		$sql="update campues_beprojects set title='$title' where id='$id'";
		if (!mysql_query($sql,$con))
		{
			die('Error: ' . mysql_error());
		}
...
<td><a href="manage_beprojects.php?action=add" class="text_content">&nbsp;&nbsp;&nbsp;&nbsp;<strong>Add New Project</strong></a> | <a href="manage_beprojects.php?action=view" class="text_content"><strong>View project</strong></a></td>
...
<?php if($_REQUEST['action']=='view') {?>
  ...
  <tr>
	...
	$result = mysql_query("SELECT * FROM campues_beprojects where 1 ".$attach." ");
  ...
  <tr>
	...
	<?php	
	  ...
	  <tr class="text_content">
		...
		<td valign="top" bgcolor="#FFFFFF"  class="text_content"><a href="be_projects/<?php echo $row['projects']; ?>" target="_blank">View</a></td>
        ...
        <?php if($row['status'] == '1') { ?><a href="manage_beprojects.php?id=<?php echo $row['id']; ?>&status=0&action=active" onClick="return(window.confirm('Are you sure to deactivate .'));"><?php echo "Deactive"; ?></a><?php } else { ?><a href="manage_beprojects.php?id=<?php echo $row['id']; ?>&status=1&action=active" onClick="return(window.confirm('Are u sure to activate .'));"><?php echo "Active"; ?></a><?php } ?>&nbsp;&nbsp; 

	   <a href="manage_beprojects.php?action=edit&id=<?php echo $row['id']?>"><img src="edit.gif" alt="Edit" width="15" height="15" border="0" /></a> &nbsp;&nbsp; <a href="manage_beprojects.php?action=del&id=<?php echo $row['id']?>" onClick="return(window.confirm('Are you sure you want to delete?'));"><img src="delete.gif" alt="Delete" width="15" height="15" border="0" /></a></td>
 ...
 <?php 
...

  {
...

	  {
			...
			$result2 = mysql_query("SELECT *  FROM campues_beprojects  where id='$_REQUEST[id]'");
